...
static NSString *const kSampleClientIDKey = @"443853727988.apps.googleusercontent.com";
static NSString *const kSampleClientSecretKey = @"aNFFq6S1RKuqZTVWi-Z0aQ-l";
...
//    self.clientIDField.text = kSampleClientIDKey;
//    self.clientSecretField.text = kSampleClientSecretKey;
  ...
  [self updateUI];
    ...
    UIBarButtonItem *butt = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(donePress)];
...
/*    
    UINavigationController* temp = [[UINavigationController alloc] initWithRootViewController:viewController];
...
- (void)loadClientIDValues {
  ...
  if ([self isGoogleSegmentSelected]) {
      self.clientIDField.text = kSampleClientIDKey;//[defaults stringForKey:kGoogleClientIDKey];
      self.clientSecretField.text = kSampleClientSecretKey;//[defaults stringForKey:kGoogleClientSecretKey];
  } else {
