...
NSString* const SVProgressHUDDidCancelNotification = @"SVProgressHUDDidCancelNotification";
...
static const CGFloat SVProgressHUDRingRadius = 20;
static const CGFloat SVProgressHUDRingThickness = 2;
...
- (void)updatePosition {
    ...
    if(string) {
        ...
        if (stringWidth < 280)
            hudWidth  = MAX(100,stringWidth + 20);
        else
            hudWidth = 300;
    ...
    if (_cancelButton){
        ...
        [_cancelButton setBackgroundColor:[self hudBackgroundColor]];
        [_cancelButton setCenter:CGPointMake(self.imageView.center.x, self.imageView.center.y + self.imageView.bounds.size.height/2 + 10 + _cancelButton.bounds.size.height/2)];
    }
}
...
- (NSDictionary *)notificationUserInfo
{
    return (self.stringLabel.text ? @{SVProgressHUDStatusUserInfoKey : (self.stringLabel.attributedText?[self.stringLabel.attributedText string]:self.stringLabel.text)} : nil);
}
...
- (CAShapeLayer *)backgroundRingLayer {
    if(!_backgroundRingLayer) {
        CGPoint center = CGPointMake(CGRectGetWidth(hudView.frame)/2, CGRectGetHeight(hudView.frame)/2);
        _backgroundRingLayer = [self createRingLayerWithCenter:center radius:SVProgressHUDRingRadius lineWidth:SVProgressHUDRingThickness
                                                         color:
                                    [UIColor colorWithWhite:0.5 alpha:1]//self.hudRingBackgroundColor
...
- (void)cancelTapped{
    [[NSNotificationCenter defaultCenter] postNotificationName:SVProgressHUDDidCancelNotification object:nil];
}
...
- (UILabel *)stringLabel {
    if (stringLabel == nil) {
        ...
        */
        ...
        [stringLabel addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(cancelTapped)]];
    }
